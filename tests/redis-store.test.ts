...
beforeEach(() => {
  ...
  getRedisNamespaceMock = vi.fn(() => 'beako:tenant')
})
...
describe('redis-store', () => {
  ...
  it('remove mensagens por jid e usa fallback seguro em caso de falha', async () => {
    ...
    expect(redis.client.hDel).toHaveBeenCalledWith('beako:tenant:store:messages', ['chat@s.whatsapp.net::0:1', 'chat@s.whatsapp.net:user@s.whatsapp.net:0:2'])
