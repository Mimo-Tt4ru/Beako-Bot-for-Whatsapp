...
FROM beako.`groups` g
LEFT JOIN beako.group_participants gp ON g.jid = gp.group_jid
...
FROM beako.messages m
LEFT JOIN beako.users u ON m.sender_user_id = u.id
...
FROM beako.users u
LEFT JOIN beako.user_identifiers ui ON u.id = ui.user_id
...
SELECT 
    ...
    mm.media_type, 
    mm.mime_type
FROM beako.message_media mm
JOIN beako.messages m ON mm.message_db_id = m.id
