...
const DEFAULT_PACK_NAME = 'Criado pelo bot Beako'
const DEFAULT_PACK_AUTHOR = 'Beako'
...
const executeStickerCommand: Command['execute'] = async (ctx) => {
  ...
  try {
    ...
    if (sticker.length >= MAX_STICKER_SIZE_BYTES) {
      await safeReply('❌ A figurinha em questão ficou com 1.5MB ou mais. Envie uma mídia menor por favor!.')
      return
    }
    ...
    await safeReply(`❌ Infelizmente não foi possível gerar a figurinha: ${reason}`)
  }
}
